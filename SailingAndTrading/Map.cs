...
namespace SailingAndTrading
{
    internal class Map
    {
        ...
        {
            var newList = coordinationList.OrderBy(x => x.YCoord).ThenBy(x => x.XCoord).ToList();
            int index = 0;
            for (int i = 0; i < 40; i++)
            {
                //生成100个字符.，生在一行，用new string方法
                string s = new string('.', 100);
                while (index < newList.Count && i == newList[index].YCoord)
                {
                    //替换新字符，用remove删除并insert添加新字符
                    s = s.Remove(newList[index].XCoord, 1).Insert(newList[index].XCoord, newList[index].Icon.ToString());
                    index++;
                }
                Console.WriteLine(s);
            }
